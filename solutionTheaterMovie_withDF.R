...
df_snacks <- data.frame(snacks)
df_snacks

snacks['popcorn']
length(snacks)
random_snack <- sample(length(snacks), 1)
random_snack
snacks[random_snack][[1]]
snacks[random_snack][[1]]
...
df_revenue_week <- data.frame(matrix(ncol = 7, nrow = length(movies)))
colnames(df_revenue_week) <- named_days_of_week
df_revenue_week[,1]
...
for (i in 1:length(week_days)) {
  ...
  for (j in 1:length(screens)) {
    ...
    print(sprintf("Cumulative revenue per movie"))
    print(screen_revenue[i])
    
    df_revenue_week[j,i] <- screen_revenue[i]
